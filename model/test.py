...
def main(argv=None):
    ...
    with tf.Session(graph=detection_graph, config=tf.ConfigProto(allow_soft_placement=True)) as detection_sess:
        detection_ckpt_state = tf.train.get_checkpoint_state(checkpoint_dir=FLAGS.checkpoint_path)
        detection_model_path = os.path.join(FLAGS.checkpoint_path, os.path.basename(detection_ckpt_state.model_checkpoint_path))
        print('Restore from {}'.format(detection_model_path))
        detection_saver.restore(detection_sess, detection_model_path)

        with tf.Session(config=config, graph=recognition_graph) as recognition_sess:
